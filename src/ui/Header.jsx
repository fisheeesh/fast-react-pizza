...
import { getCartItems, getTotalCartQuantity } from "../features/cart/cartSlice";

export default function Header() {
    const navigate = useNavigate()
    const totalCartQuantity = useSelector(getTotalCartQuantity)
    const cartLength = useSelector(getCartItems)
        ...
        <header className="flex items-center justify-between bg-yellow-400 px-4 py-3 uppercase border-b border-stone-300 sm:px-6">
            ...
            <div className="flex items-center gap-2">
                ...
                <div className="relative">
                    ...
                    {totalCartQuantity > 0 && <span className="absolute left-2.5 top-0.5 w-3 h-3 flex items-center justify-center text-[9px] text-white bg-red-600 rounded-full">{cartLength}</span>}
                </div>
            </div>
